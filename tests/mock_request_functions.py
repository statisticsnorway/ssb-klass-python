...
def correspondance_table_by_id_success(mock_response):
    mock_response.return_value = (
        mock_response_data.correspondance_table_by_id_fake_data()
    )
...
def changes_success(mock_response):
    mock_response.return_value = mock_response_data.changes_fake_data()
    return klass_requests.changes("0", "2023-01-01")


@mock.patch.object(requests.Session, "send")
def classificationfamilies_success(mock_response):
    mock_response.return_value = mock_response_data.classificationfamilies_fake_data()
...
def classificationfamilies_by_id_success(mock_response):
    mock_response.return_value = (
        mock_response_data.classificationfamilies_by_id_fake_data()
    )
...
def sections_list_success(mock_response):
    mock_response.return_value = mock_response_data.sections_fake_data()
