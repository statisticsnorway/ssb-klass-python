...
def variant_df(variant_id: int, language="nb") -> pd.DataFrame:
    url = (BASE_URL + "variants/" + 
           str(variant_id) + "?language=" + language)
    headers = {'Accept': 'application/json'}
    print(url)
    return klass_df(url, level="classificationItems")


def classification_codes_df(classification_id: int, 
                         from_date: str = "", 
                         to_date: str = "", 
                         include_future: bool = True) -> pd.DataFrame:
    url = (BASE_URL + "classifications/" + 
           str(classification_id) + "/codes.json?")
    url_params = []
    if from_date: url_params += [f"from={validate_date(from_date)}"]
    if to_date: url_params += [f"from={validate_date(to_date)}"]
    if include_future: url_params += ["includeFuture=True"]
    url += "&".join(url_params)
    print(url)
...
def search_classifications(searchterm: str, 
                           page: int = 0, 
                           size: int = 20,
                           ssb_section: int = 0) -> list:
    url = (BASE_URL + "classifications/search?query=" + 
           searchterm +
          "&page=" + str(page) +
          "&size=" + str(size))
    if ssb_section: url += f"&ssbsection={ssb_section_longname(ssb_section).replace(' ', '%20')}"
    print(url)
    headers = {'Accept': 'application/json'}
...
def find_changes(classification_id: int,
                 from_date: str,
                 to_date: str) -> list():
    url = (BASE_URL + "classifications/" + 
           str(classification_id) + "/changes.json?from=" +
           validate_date(from_date) + "&to=" + 
